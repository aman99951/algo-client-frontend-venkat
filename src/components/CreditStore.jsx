...
export default function CreditStore({ apiKey, isOpen, onClose }) {
  ...
  const tierColors = {
    starter: { bg: 'from-blue-500/20 to-blue-600/10', border: 'border-blue-500/20', text: 'text-blue-400', btn: 'bg-blue-600 hover:bg-blue-700' },
    pro: { bg: 'from-purple-500/20 to-purple-600/10', border: 'border-purple-500/30', text: 'text-purple-400', btn: 'bg-purple-600 hover:bg-purple-700' },
    elite: { bg: 'from-amber-500/20 to-amber-600/10', border: 'border-amber-500/20', text: 'text-amber-400', btn: 'bg-amber-600 hover:bg-amber-700' },
    ultra: { bg: 'from-emerald-500/20 to-emerald-600/10', border: 'border-emerald-500/20', text: 'text-emerald-400', btn: 'bg-emerald-600 hover:bg-emerald-700' },
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-3xl mx-4 max-h-[90vh] overflow-y-auto bg-[#0f1629] border border-white/10 rounded-2xl shadow-2xl">
        {/* Header */}
        <div className="sticky top-0 z-10 bg-[#0f1629]/95 backdrop-blur border-b border-white/10 px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">
              Current balance: <span className="text-white font-semibold">{balance.toLocaleString()}</span> credits
            </p>
          </div>
        ...
        <div className="flex gap-1 px-6 pt-4">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
              activeTab === 'packages' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-white/5'
            }`}
          >
            Buy Credits
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-1.5 ${
              activeTab === 'history' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-white/5'
            }`}
          >
        ...
        {activeTab === 'packages' ? (
          <div className="p-6">
            ...
            <div className="flex flex-wrap gap-4 mb-6 text-xs text-gray-400">
              <span className="flex items-center gap-1"><Shield className="w-3.5 h-3.5 text-green-400" /> Secure Razorpay Payments</span>
              <span className="flex items-center gap-1"><Zap className="w-3.5 h-3.5 text-yellow-400" /> Instant Credit</span>
              <span className="flex items-center gap-1"><TrendingUp className="w-3.5 h-3.5 text-blue-400" /> No Expiry</span>
            </div>
                  ...
                  return (
                    <div
                      ...
                      className={`relative bg-gradient-to-b ${colors.bg} border ${colors.border} rounded-xl p-5 flex flex-col transition-all hover:scale-[1.02] hover:shadow-lg ${
                        pkg.is_popular ? 'ring-2 ring-purple-500/50' : ''
                      }`}
                    >
                      {/* Popular Badge */}
                      {pkg.is_popular && (
                        <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 bg-purple-600 rounded-full text-[10px] font-bold text-white uppercase tracking-wider">
                          Most Popular
                        </div>
                      )}
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        <div className={`w-10 h-10 rounded-xl bg-black/20 flex items-center justify-center ${colors.text}`}>
                      ...
                      <div className="mb-4">
                        <div className="flex items-baseline gap-1">
                          <span className="text-3xl font-bold text-white">₹{pkg.price_inr.toLocaleString()}</span>
                        </div>
            ...
            {history.length === 0 ? (
              ...
              <div className="space-y-2">
                {history.map((entry, i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between px-4 py-3 bg-white/3 rounded-lg border border-white/5"
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                        entry.type === 'credit' ? 'bg-green-500/20' : 'bg-red-500/20'
                      }`}>
                        {entry.type === 'credit'
                      ...
                      <div>
                        <div className="text-sm text-white">{entry.description || entry.reason}</div>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-gray-500">bal: {entry.balance_after}</div>
                    </div>
                  </div>
                ))}
              </div>
